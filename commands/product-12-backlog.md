...
- `./artifacts/product-requirements.md` — the requirements doc from `/product-8-engineering-handoff`
...
```
YYYY-MM-DD HH:MM PST — /product-12-backlog — ./artifacts/backlog/ (N items)
```
...
5. Append another log line: `YYYY-MM-DD HH:MM PST — /product-12-backlog — pushed N items to <tool>`.
...
- If the toolchain push needs to be re-run later (e.g., they haven't picked a tracker yet): "When you pick an issue tracker, update `./artifacts/toolchain.md` and re-run `/product-12-backlog` — it'll skip the local writes and go straight to the push."
