...
- If missing: tell the user "No `./artifacts/product-log.md` found — this product hasn't started the playbook yet. Run `/0-intro` to orient, then `/1-kickoff` to begin." Stop here.
- If present: parse each line. Lines look like `YYYY-MM-DD HH:MM <TZ> — /N-name — <artifact path or note>` (older logs may use `/product-N-name` — accept both).
...
**Last log entry:** 2026-05-14 09:32 PST — /5-mvp-scope — ./artifacts/mvp-scope.md

## Phases completed
- ✅ /1-kickoff — principles.md, toolchain.md
- ✅ /2-vision — PRFAQ.md, one-pager.md
- ✅ /3-gtm — gtm-plan.md
- ✅ /5-mvp-scope — mvp-scope.md
- ⏭️  /4-design — skipped (no artifact)
- ⏳ /7-stress-test — not yet run on mvp-scope.md (recommended gate)
...
## Recommended next
Run `/7-stress-test ./artifacts/mvp-scope.md` — this is the natural antagonist gate after MVP scope is locked. POV lineup: Sponsor, Technical, Operator, Customer.
...
- **Recommended next** — single concrete command with one short sentence of justification. Pick based on:
  ...
  - If MVP stress-test is done but no go/no-go yet → recommend `/8-go-no-go`.
...
No file is created or modified. No log entry is appended. `/status` is a pure read.
